...
$fullAddressCopyTo .= $address_line1.$address_line2.$city.$state_name.$pincode.'<br>';
	...
	$left_file_path=$_SESSION['base_url']."/letter/samples/left/left_".$_SESSION['letter_type_name'].".txt";
	$left_data = file_get_contents($left_file_path); //read the file
	$left_convert = explode("\n", $left_data); //create array separate by new line
	
	$right_file_path=$_SESSION['base_url']."/letter/samples/right/right_".$_SESSION['letter_type_name'].".txt";
	$right_data = file_get_contents($right_file_path); //read the file
	$right_convert = explode("\n", $right_data); //create array separate by new line
...
//print_r($convert);
	 for($i=0;$i<count($convert);$i++)
	 {
		...
		if("aaj ki date"==trim($convert[$i]))
			echo date("d-m-Y");
		else
		echo $convert[$i];	
	 }
echo '<div style="width:100%">';
	echo '<div style="float: left; margin:0px; width:35%;">';
	 for ($i=0;$i<count($left_convert);$i++)
	 {
	 echo $left_convert[$i];
	 }
	 echo '</div>';
	 echo '<div style="float: right;  margin:0px;  width:35%;">';
	 for ($i=0;$i<count($right_convert);$i++)
	 {
	echo $right_convert[$i];
	 }
	 echo '</div>';
	 echo '</div><div style="clear:both;"></div>';
	 
echo '<div style="width:100%;">';
	echo '<p id="title"   style="text-align:center;font-size:18px;padding:0px; font-weight: bold;">'. $letterTypes[0]['title_file_path'] .'</p>';
	echo '<p>Letter No. :- '.$letterTypes[0]['letter_no'].'</div><div style="text-align:right;">Date:- '. date('d-m-Y').'</div><div style="clear:both;"></div>';
	echo '<p>'.
	'To :- '.$applicantTo.'<br>'. 
...
echo '</div>';
   ...
   {   
	...
	echo '<p><a id="download" href="inlineall.php?action=download" class="links" onclick="hide()">Download</a></p>'; 
	echo '<p><a id="download" href="/sclsc1/pages/dealing_assistant/generateLetterDetails.php?appliId='.$_SESSION['appliId'].'&msg=Please Regenerate Letter OR Send to Dispatch">Back</a></p>';
	echo '</body></html>';
 // session_destroy();
?>