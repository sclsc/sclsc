...
$fullAddressCopyTo .= $address_line1.$address_line2.$city.$state_name.$pincode.'<br>';
    ...
    $header_file_path=$_SESSION['base_url']."/letter/samples/header/letter-test.doc";
...
/*$fp = fopen($header_file_path, "rb");
$contents_doc = fread($fp, filesize($header_file_path));
fclose($fp);
echo $contents_doc;
*/

	
	$file = fopen($header_file_path,'rb');
...
echo '</head>	<body>';	
	...
	echo '<br/>	<p id="subject">Subject :- '.$letterTypes[0]['subject_file_path'].'</p>';
