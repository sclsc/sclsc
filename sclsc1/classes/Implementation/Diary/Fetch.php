<?php
	...
	class Fetch implements IFetch\Fetchable
	{
		...
		public function getAllDiary($limit,$start)
		{
			...
			try{
				$query = "SELECT d.id, d.diary_no, c.category_name, d.letter_no, d.applicant, d.recieved_date,
						...
						d.sender_city,d.is_taken,s.state_name, d.pincode,d.recieved_through, u.user_name
		...
		//	echo $fromDate.'==='.$toDate.'====='.$category.'==='.$receivedThrough.'======='.$state; exit;
			$count = 0;
			$diary =array();
			try{
				$con = Conn\Connection::getConnection();
				$query = "SELECT d.id, d.diary_no, c.category_name, d.letter_no, d.applicant, d.recieved_date,
						...
						join tbl_diary_category c on c.id = d.category_id 
						JOIN tbl_user u ON d.mark_to = u.id 
						WHERE d.recieved_date BETWEEN :fromDate AND :toDate";
				
				if($category!=0)
				{
					$query.=" AND d.category_id=:category_id";						
				}
				if($receivedThrough!=0){
					$query.=" AND d.recieved_through =:recieved_through";						
				}				
				if($state!=0){
					$query.=" AND d.sender_state =:sender_state";						
				}
				$query.=" ORDER BY  d.id, d.diary_no asc LIMIT :limit OFFSET :offset";
				...
				if($receivedThrough!=0){
				$stmt->bindValue(':recieved_through', $receivedThrough, \PDO::PARAM_INT);
				}
				if($state!=0){
				$stmt->bindValue(':sender_state', $state, \PDO::PARAM_INT);
				}
				...
				$state )
		{
			...
			try{
				$con = Conn\Connection::getConnection();
				$query = "SELECT count(*) count FROM tbl_diary 
						where recieved_date between :fromDate AND :toDate";
				if($category!=0)
				{
					$query.=" AND category_id=:category_id";
				}
				if($receivedThrough!=0){
					$query.=" AND recieved_through =:recieved_through";
				}
				if($state!=0){
					$query.=" AND sender_state =:sender_state";
				}
		...
		public function getSpecificDiary(
				$fromDate,
				$toDate,
				$userId,
				$receivedThrough,
		...
		public function getSpecificDiaryCount(
				$fromDate,
				$toDate,
				$userId,
				$receivedThrough,
		...
		public function getSpecificDiaryDownload(
				$fromDate,
				$toDate,
				$userId,
				$receivedThrough,
				$state )
		{
			...
			try{
				...
				$query.=" ORDER BY  d.id, d.diary_no asc ";
		...
		public function getNextDiaryId()
		{
			...
			{
				$year = $currentYear;
				$nextGeneratedDiaryNumber = '1'.$year;
			}
				...
				{				
					...
					try{
						$query = "SELECT a.id, a.applicant_name, a.applicant_contact_no,a.applicant_mobile_no,
								...
								FULL JOIN tbl_applicant_address b ON a.id = b.applicant_id
							   FULL JOIN tbl_states c ON	b.applicant_state = c.id
