<?php
	...
	require_once $_SESSION['base_url'].'/classes/Interfaces/Advocate/Addable.php';
	
	use classes\Connection as Conn;
	use classes\Interfaces\Advocate as IAdd;
	
	class Add implements IAdd\Addable
	{
		public function reg_advocate(
				$full_name,
				$gender,
				$lang,
				$email_id,
		...
		try 
		 {
			...
			$sqlQuery = "insert into tbl_advocates
				        (advocate_name, advocate_code,email_id, contact_no1, contact_no2, advocate_enrol_date,
			...
			$advocate_maxID = $con->lastInsertId('tbl_advocates_id_seq');		
			...
			$period->bindParam(':advocate_maxID', $advocate_maxID, \PDO::PARAM_INT);
			...
			$comm_add_1='f';
			$sqladd = "insert into tbl_adv_address (advocates_id,address1,address2,city,state,pincode,is_commun_address)
			...
			if($comm_add=='1')
			 {									
			$sqlcomm = "insert into tbl_adv_address
			           (advocates_id, address1, address2, city, state, pincode, is_commun_address, commun_name)
			           values(:advocate_maxID, :c_address1, :c_address2, :c_city, :c_state, :c_pincode, :is_commun_address, :c_name)";
