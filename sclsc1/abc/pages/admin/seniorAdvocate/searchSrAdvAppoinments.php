...
error_reporting(0);
	...
	use classes\Implementation\SeniorAdvocate as impAdvocate;
	...
	$targetpage = 'searchSrAdvAppoinments.php';
	$adjacents = 2;
	$limit = 10;
	$start = 0;
	$page = 0;
	...
	if(isset($_GET['page']))
	{
		...
		if($page){
	     ...
	     $allAdvocateList = $fetchAdvocate->getSrAdvocatesList();

	     
     if(isset($_GET['submit']))
     {
     	$alladvocate = $fetchAdvocate->searchSrAdvocatesOnPannel($_GET['fromDate'], $_GET['toDate'], (int)$_GET['advocate_name'], $limit, $start);
     	$Records = $fetchAdvocate->searchSrAdvocatesOnPannelCount($_GET['fromDate'], $_GET['toDate'], (int)$_GET['advocate_name']);
		     			...
		     			<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="GET">
		     					...
		     					<select name="advocate_name" >
		     						<option value="0">All Advocate</option>
										...
										{											
										?>										
										<option value="<?php echo $allAdvocateList[$i]['id']; ?>" <?php if($allAdvocateList[$i]['id']==$_GET['advocate_name'] || $allAdvocateList[$i]['id']==$_GET['advocate_name']) { echo "selected=selected"; } ?> ><?php echo $allAdvocateList[$i]['sr_advocate_name']; ?></option>
										<?php 
										}
										?>
		     					</select>
	     			...
	     			<div style="border:2px solid #4b8df8;padding:7px;">
		     			<table id="table-records" cellpadding="0px" cellspacing="0px" width="100%">
							...
							?>
	     					...
	     					<td><?php echo $i+1; ?></td>	     					
	     					 <td><?php echo $alladvocate[$i]['sr_advocate_name']; ?></td>
	     					 <td><?php echo $alladvocate[$i]['sr_advocate_code']; ?></td>
	     					<td><?php echo $alladvocate[$i]['count']; ?></td>
	     					<td><a href="viewSrAdvAppoinmentDetails.php?advocateId=<?php echo $alladvocate[$i]['id']; ?>" >View Applications</a></td>                           
