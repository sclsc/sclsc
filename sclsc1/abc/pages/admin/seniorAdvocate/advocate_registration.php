<?php 
	...
	require_once $_SESSION['base_url'].'/classes/Implementation/SeniorAdvocate/Add.php';
	...
	$addSrAdvocate = new impSrAdvocate\Add();
	...
	if(count($errMsg) == 0)
	 {
		...
		if ($title == "" || $full_name == "" || $gender == "" || $flag != 1)
			...
			$reg_sr_advocate = $addSrAdvocate->reg_sr_advocate( 
					$full_name, 
					$gender, 
					$lang, 
					$email_id, 
...
if(isset($_POST['update']))
{
	...
	*/
	$abc = $editSrAdvocate->upSrAvocateStatus($advocateId,$full_name,$_POST ['gender'],$_POST ['lang'],$_POST ['email_id'],$_POST ['mobile_no'],$_POST ['mobile_no2'],$date_of_enrol,$name_of_bar,$date_of_scba,
			$aor,$_POST ['advocate_code'],$_POST ['is_on_panel'],$_POST ['is_aor'],$_POST ['address1'],$_POST ['address2'],$_POST ['city'],
			$_POST ['state'],$_POST ['pincode'],$_POST ['c_name'],
			$_POST ['c_address1'],$_POST ['c_address2'],$_POST ['c_city'],$_POST ['c_state'],$_POST ['c_pincode'],$start_date1,
			$end_date1,$hiddenData,$date_of_reg);
	if ($abc==1)
	{
		$msg = "Senoir Adovcate Has been Updated successfully.";
		header("location:index.php?msg=$msg");
	}
	else
	{
...
$SenoirAdvocateId=$_GET['srAdvocateId'];
$Senoiradvocatedetails=$fetchSrAdvocate->singleSrAvocateDetails($SenoirAdvocateId);
//print_r($Senoiradvocatedetails);
$SenoirAdvoRegDate=$fetchSrAdvocate->SenoirAdvocateRegPeriod($SenoirAdvocateId);
$fullName=$Senoiradvocatedetails[0]['advocate_name'];
$a=explode(' ', $fullName);
//print_r($SenoirAdvoRegDate);
...
.ui-datepicker {
	    ...
	    <div class="wrapper" >
			...
			<div id="right-wrapper">
				<div id="breadcrumb">
	     			<ul>
	     				<li><a href="#">Home /</a></li>
	     				<li><a href="index.php">SrAdvocate Registration</a></li>
	     		...
	     		<div class="title1" style="height:20px;">
     				<div id="left-title">Senior Advocate Registration</div>
     		    ...
     		    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" name="advo_regis" id="advo_regis"
     		     method="post" onsubmit="return validateAdvocateForm()">
					...
					<div class="left">
							...
							<div class="right">
								<input type="text" placeholder="Full Name" name="full_name"  maxlength="50" value="<?php if(isset($Senoiradvocatedetails[0]['advocate_name'])) { echo substr($fullName,3); } else echo ''; ?>"/>
							</div>			
							...
							<div class="right">
								...
								onKeyPress="return alphadot_hyphendigit_No_Limited(event)" value="<?php if(isset($Senoiradvocatedetails[0]['language'])) { echo $Senoiradvocatedetails[0]['language']; } else echo ''; ?>"/>
							</div>
							<div id="divAor">
							<div class="clear"></div>
							<div class="left" >Sr. Advocate Code </div>
							<div class="right">
								<input type="text" placeholder="Advocate Code" name="advocate_code" maxlength="10" value="<?php if(isset($Senoiradvocatedetails[0]['advocate_code'])) { echo $Senoiradvocatedetails[0]['advocate_code']; } else echo ''; ?>"/>
							</div>
							
							<div class="clear"></div>
							<div class="left">AOR Designate Date </div>
							<div class="right">
								<input type="text" placeholder="DD-MM-YYYY" name="aor" class="date1" id="aor" onblur="aorDate(advo_regis)" value="<?php if(isset($Senoiradvocatedetails[0]['aor_desig_date'])) { $myFormatForView=  date("d-m-Y", strtotime($Senoiradvocatedetails[0]['aor_desig_date']));echo $myFormatForView;} else echo ''; ?>"/>
							</div>
							</div>
					</div>					
					...
					<div class="right">							
							...
							<div class="right">
								<input type="radio" name="gender" value="Male" <?php if(isset($Senoiradvocatedetails[0]['gender'])) { echo 'checked'; } ?> /> Male 
								<input type="radio" name="gender" value="Female" <?php if(isset($Senoiradvocatedetails[0]['gender'])) { echo 'checked'; } ?> /> Female																
							</div>
							
							<div class="clear"></div>
							<div class="left">E-mail ID </div>
							<div class="right">	
							<input type="text" placeholder="Email Id" name="email_id" id="email_id"  value="<?php if(isset($Senoiradvocatedetails[0]['email_id'])) { echo $Senoiradvocatedetails[0]['email_id']; } else echo ''; ?>"/>
							</div>
							...
							<div class="right">
								...
								onKeyPress="return number(event)" name="mobile_no" id="mobile_no" value="<?php if(isset($Senoiradvocatedetails[0]['contact_no1'])) { echo $Senoiradvocatedetails[0]['contact_no1']; } else echo ''; ?>"  />
							</div>
							<div class="clear"></div>
							<div class="left">Other Contact No.</div>
							<div class="right">
								<input type="text" placeholder=" Alternative number "  maxlength="12" 
								onKeyPress="return number(event)" name="mobile_no2" id="mobile_no2" value="<?php if(isset($Senoiradvocatedetails[0]['contact_no2'])) { echo $Senoiradvocatedetails[0]['contact_no2']; } else echo ''; ?>"/>
							</div>							
					</div>					
				</div>
				...
				<div class="clear"></div>
					...
					<div class="clear"></div>
					<div class="left">
							<div class="left">Enrolment Date as Sr. Advocate<span class="red">*</span></div>
							<div class="right">
							<input type="text" placeholder="DD-MM-YYYY"	 class="date1" name="date_of_enrol" id="date_of_enrol" onblur="enrolDate(advo_regis)" value="<?php if(isset($Senoiradvocatedetails[0]['advocate_enrol_date'])) { $advocateEnrolDate=  date("d-m-Y", strtotime($Senoiradvocatedetails[0]['advocate_enrol_date']));echo $advocateEnrolDate; } else echo ''; ?>"/>							
							</div>
							
							<div class="clear"></div>
							<div class="left">Enrolment Date(SCBA) <span class="red">*</span></div>
							<div class="right">
								<input type="text" placeholder="DD-MM-YYYY" name="date_of_scba" class="date1" id="date_of_scba" id="date_of_scba" onblur="enrolScbaDate(advo_regis)" value="<?php if(isset($Senoiradvocatedetails[0]['scba_enrol_date'])) {$ScbaEnrolDate=  date("d-m-Y", strtotime($Senoiradvocatedetails[0]['scba_enrol_date']));echo $ScbaEnrolDate;} else echo ''; ?>"/>
							</div>
							
							<div class="clear"></div>
							<div class="left">Registration Date <span class="red">*</span> </div>
							<div class="right">
								<input type="text" placeholder="DD-MM-YYYY"	name="date_of_reg" id="date_of_reg" class="date1" onblur="regDate(advo_regis)" value="<?php if(isset($SenoirAdvoRegDate[0]['start_date'])) { $advocateRegistrationDate=  date("d-m-Y", strtotime($SenoirAdvoRegDate[0]['start_date']));echo $advocateRegistrationDate; } else echo ''; ?>"/>
							</div>							
					</div>					
					...
					<div class="right">
							...
							<div class="right">
								<input type="text" placeholder="Enrolled Bar Council" name="name_of_bar" maxlength="100" value="<?php if(isset($Senoiradvocatedetails[0]['enrolled_bar'])) { echo $Senoiradvocatedetails[0]['enrolled_bar']; } else echo ''; ?>"/>
							</div>
        ...
        <div class="myTemplate" style="display:none">
          <p>
            ...
            <a href="#"  class="remove" id="remNew">Remove</a> </p>
				...
				<div class="clear"></div>
					...
					<div class="left">
							...
							<div class="right">
							<input type="text" name="address1" id="address1" placeholder="Address Line 1" maxlength="50" value="<?php if(isset($Senoiradvocatedetails[0]['address1'])) { echo $Senoiradvocatedetails[0]['address1']; } else echo ''; ?>"/>							
							</div>
							
							<div class="clear"></div>
							<div class="left">Address Line 2</div>
							<div class="right">
								<input type="text" name="address2" id="address2" placeholder="Address Line 2" maxlength="50" value="<?php if(isset($Senoiradvocatedetails[0]['address2'])) { echo $Senoiradvocatedetails[0]['address2']; } else echo ''; ?>"/>
							</div>
							
							<div class="clear"></div>
							<div class="left">City <span class="red">*</span></div>
							<div class="right">
								<input type="text" placeholder="City" name="city" id="city" maxlength="20"
								onKeyPress="return alphadot_hyphendigit_No_Limited(event)" value="<?php if(isset($Senoiradvocatedetails[0]['city'])) { echo $Senoiradvocatedetails[0]['city']; } else echo ''; ?>"/>
							</div>
					...
					<div class="right">
							...
							<select name="state" id="state">
									...
									for($i = 0; $i < count ( $states ); $i ++) {
										?>
										<option value="<?php echo $states[$i]['id']; ?>" <?php if($Senoiradvocatedetails[0]['state']==$states[$i]['id'])  echo "selected=selected"; ?>><?php echo $states[$i]['state_name']; ?></option>
									<?php
									}
									?>
									</select>
							...
							<div class="right">
								<input type="text" placeholder="Pin Code" name="pincode" id="pincode" maxlength="6"
									onKeyPress="return number(event)" value="<?php if(isset($Senoiradvocatedetails[0]['pincode'])) { echo $Senoiradvocatedetails[0]['pincode']; } else echo ''; ?>"/>
							</div>	
					</div>
				...
				<div class="clear"></div>
					<h3><input type="checkbox" name="comm_add" id="comm_add" style="height: 30px;" class="first" value="1" <?php if(isset($Senoiradvocatedetails[1]['is_commun_address'])=='1') { echo 'checked'; }?> onclick="showMe('hide', this)" />  
					...
					<div class="left">
							...
							<div class="right">
							<input type="text" name="c_name" id="c_name" placeholder="Full Name" maxlength="50" value="<?php if(isset($Senoiradvocatedetails[1]['is_commun_address'])=='1') { echo $Senoiradvocatedetails[1]['commun_name']; } else echo ''; ?>"/>							
							</div>
							
							<div class="clear"></div>
							<div class="left">Address Line 1 <span class="red">*</span></div>
							<div class="right">
							<input type="text" name="c_address1" id="c_address1" placeholder="Address Line 1" maxlength="50" value="<?php if(isset($Senoiradvocatedetails[1]['is_commun_address'])=='1') { echo $Senoiradvocatedetails[1]['address1']; } else echo ''; ?>"/>							
							</div>
							
							<div class="clear"></div>
							<div class="left">Address Line 2</div>
							<div class="right">
								<input type="text" name="c_address2" placeholder="Address Line 2" maxlength="50" value="<?php if(isset($Senoiradvocatedetails[1]['is_commun_address'])=='1') { echo $Senoiradvocatedetails[1]['address2']; } else echo ''; ?>"/>
							</div>
					...
					<div class="right">
							...
							<div class="right">
								<input type="text" placeholder="City" name="c_city"
										onKeyPress="return alphadot_hyphendigit_No_Limited(event)" maxlength="50" value="<?php if(isset($Senoiradvocatedetails[1]['is_commun_address'])=='1') { echo $Senoiradvocatedetails[1]['city']; } else echo ''; ?>"/>
							</div>								
							...
							<select name="c_state" id="c_state">
									...
									for($i = 0; $i < count ( $states ); $i ++) {
										?>
										<option value="<?php echo $states[$i]['id']; ?>" <?php if($Senoiradvocatedetails[1]['state']==$states[$i]['id'])  echo "selected=selected";  ?>><?php echo $states[$i]['state_name']; ?></option>
									<?php
									}
									?>
									</select>								
							...
							<div class="right">
								<input type="text" placeholder="Pin Code" name="c_pincode" id="c_pincode" maxlength="6"
									onKeyPress="return number(event)" value="<?php if(isset($Senoiradvocatedetails[1]['pincode'])) { echo $Senoiradvocatedetails[1]['pincode']; } else echo ''; ?>"/>
							</div>
			...
			<div class="clear"></div>			
	     			...
	     			<?php if(isset($_GET['srAdvocateId']) && $_GET['action']=="edit"){?>
				     					<input type="submit" name="update" value="Update" class="form-button" />
				     				<!--  <input type="submit" name="cancel" value="Cancel" class="form-button" />-->	
				     					<input type="hidden" id="srAdvocateId" name="srAdvocateId" 
				     					value="<?php echo $_GET['srAdvocateId'];?>"/>	
				     					<?php }else {?>					
 ...
 <?php if(isset($Senoiradvocatedetails[1]['is_commun_address'])=='1') {?>
