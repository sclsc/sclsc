	...
	$fetchAdvocate = new impAdvocate\Fetch();
		...
		if(isset($_POST['submitDispatch']))
		 {		 	
			 ...
			 $isSrAdv =TRUE;
			 $applicationId = $addCof->addSrAdvocateApponment($application_id, $app_date, $adv_name, $isSrAdv);
										...
										<div class="right" >
											<select name="adv_name" id="adv_nemae" multiple style="height:30%" required="required">				     							
				     						...
				     						for($i =0;$i<count($alladvocateList);$i++)
												{
											?>
				     							<option value="<?php echo $alladvocateList[$i]['id']; ?>"><?php echo $alladvocateList[$i]['sr_advocate_name']; ?></option>
				     						<?php 
												}
				     						?>
				     						</select>
