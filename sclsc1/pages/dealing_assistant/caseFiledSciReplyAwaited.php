<?php 
	...
	$flag = $editCof-> upApplicationStatus($_GET['applicationId'], $stage_id, $sub_stage_id);
		...
		if ($flag== 1)
		{
			...
			header("Location:caseFiledSciReplyAwaited.php?msg=$msg");
		}
		else
		{
			$errmsg = "failed ?? Please try again Later.";
			header("Location:caseFiledSciReplyAwaited.php?errmsg=$errmsg");
		}
	
	} 
	
	$targetpage = 'primaFacieReplyAwaited.php';
	$adjacents = 2;
	$limit = 10;
	$start = 0;
	$page = 0;
	    ...
	    <div class="wrapper" >
			...
			<div id="breadcrumb">	     			
	     		    <ul>
	     				...
	     				<li><a href="#">Case Filed in SCI Reply Awaited</a></li>
	     			</ul>
	     		</div>
		     				...
		     				<table id="table-records">
								...
								?>
				     				<tr class="<?php echo $class; ?>" align="center">
				     					...
				     					?>
				     					<a href="caseFiledSciReplyAwaited.php?applicationId=<?php echo $diary[$i]['id'] ?>&action=up"  onclick="return confirm('Are you sure you want to go to Secand Opinion?')">Second Opinion</a>
				     					<?php
				     					}
				     					else 
				     					{
