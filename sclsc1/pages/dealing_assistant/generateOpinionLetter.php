...
if ($_POST['slpAction']=="Advocate")
{
	$allList = $fetchAdvocate->getAdvocatesList();
}
else 
{
	$allList = $fetchAdvocate->getLSCList();
}
								...
								<div class="right">
									...
									<select name="to" id="to" multiple style="height:30%" required="required">				     							
				     						...
				     						for($i =0;$i<count($allList);$i++)
												{
											?>
				     							<option value="<?php echo $allList[$i]['id']; ?>"><?php echo $allList[$i]['advocate_name']; ?></option>
				     						<?php 
												}
				     						?>
				     						</select>
