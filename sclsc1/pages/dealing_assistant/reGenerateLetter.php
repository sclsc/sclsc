...
$errmsg = '';
		...
		if(isset($_POST['reGenerateLetter']))
		 {		 	
		 //	print_r($_POST); exit;
		 	 $application_id = (int)trim($_POST['application_id']);	
		 	 $letterId       = (int)trim($_POST['letterId']);
		 ...
		 if(count($errMsg) == 0)
		 {
			 ...
			 $appliId = $editCof->reGenerateLetter(
			 		$application_id,
			 		$letterId,
			 		$letterType,			 				 		
			 		$subject,
...
$allApplicants = $fetchCof->getApplicats($application_id);
$letterTypes = $fetchLetter->getAllLetterTypes();
$GeneratedLetter = $fetchCof->getGeneratedLetter($application_id);
$letterAddresseTo  = $fetchCof->getLetterAddresseTo($GeneratedLetter[0]['id']);
$letterAddresseCopyTo = $fetchCof->getLetterAddresseCopyTo($GeneratedLetter[0]['id']);
//print_r($GeneratedLetter);
//echo $GeneratedLetter[0]['letter_type'];
?>
...
<!-- 	<script>
	...
	<?php require_once $_SESSION['base_url'].'/include/header.php';?>
			...
			<div id="right-wrapper">
				<div id="breadcrumb">
	     			<ul>
	     				...
	     				<li><a href="#">Regenerate Letter</a></li>
	     			</ul>
	     		</div>
	     		<div class="clear"></div>
	     		<?php if($msg != '')
	     		...
	     		<div class="title1" style="height:20px;">
     				<div id="left-title">Regenerate Letter</div>
	            ...
	            id="dispatch_detail" method="post" onsubmit="return validateCOF()">
							<h3>Dispatch Details</h3>
	     					<div style="width:80%;margin-left:1%">
								...
								<div class="right">
										...
										<select name="letterType" id="letterType" onchange="change_letter_type(this.value);">
				     						...
				     						for($i =0;$i<count($letterTypes);$i++)
												{
												?>
				     								<option value="<?php echo $letterTypes[$i]['id']; ?>" <?php if ($letterTypes[$i]['id']==$GeneratedLetter[0]['letter_type']) echo "selected=selected";?>>
				     								<?php echo $letterTypes[$i]['letter_type_name']; ?></option>
				     							<?php 
												}
				     						?>
				     						</select>											
									...
									<?php 	for($i=0;$i<count($allApplicants);$i++) { ?>
											<input type="checkbox" id="to"  name="to[]" 
											value="<?php echo $allApplicants[$i]['id']?>" <?php if(in_array($allApplicants[$i]['id'], $letterAddresseTo)) { echo "checked=checked"; } ?>/>
											&nbsp;&nbsp;<?php echo $allApplicants[$i]['applicant_name']?><br/>
											<?php } ?>
										</div>
								...
								<div class="right">
										...
										<div class="right">
									<?php 	for($i=0;$i<count($allApplicants);$i++) { ?>
											<input type="checkbox" id="copyTo"  name="copyTo[]" 
											value="<?php echo $allApplicants[$i]['id']?>"  <?php if(in_array($allApplicants[$i]['id'], $letterAddresseCopyTo)) { echo "checked=checked"; } ?>/>
											&nbsp;&nbsp;<?php echo $allApplicants[$i]['applicant_name']?><br/>
											<?php } ?>
										</div>
			     	...
			     	<div class="clear"></div>
				     			...
				     			<div style="text-align:center;padding:10px 0;">	
				     					...
				     					value="<?php echo $GeneratedLetter[0]['id'];?>"/>					
									<input class="form-button" type="submit" name="reGenerateLetter" value="Regenerate Letter" />
		...
		window.onload=change_letter_type(<?php echo $GeneratedLetter[0]['letter_type']; ?>);	
		</script>	
