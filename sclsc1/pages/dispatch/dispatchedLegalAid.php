<?php 
	...
	require_once $_SESSION['base_url'].'/classes/Implementation/Dispatcher/Edit.php';
	...
	$editDisp      = new impDispatcher\Edit();
	
	if (isset($_GET['applicationId']) && $_GET['action']='fileClose')
	{		
		$stageId=5;
		$subStageId=10;
		$flag = $editDisp->closeMiscApplication($_GET['applicationId'], $stageId, $subStageId);

		if ($flag == 1)
		{
			$msg = "Dairy Has been Closed successfully.";
			header("Location:dispatchedLegalAid.php?msg=$msg");
		}
		else
		{
			$errmsg = "failed ?? Please try again Later.";
			header("Location:dispatchedLegalAid.php?errmsg=$errmsg");
		}
	}
	    ...
	    <div class="wrapper" >
			...
			<div id="breadcrumb">
	     			<ul>
	     				...
	     				<li><a href="#">Dispatch Other File</a></li>
	     			</ul>
	     		</div>
	     		...
	     		<div id="breadcrumb">
	     			<div class="title1" style="height:20px;">
	     				<div id="left-title">Dispatched Dairy</div>
		     				...
		     				<table id="table-records">
								...
								?>
				     				<tr class="<?php echo $class; ?>" align="center">
				     					...
				     					<td> <a href = "dispatchedLegalAid.php?applicationId=<?php echo $diary[$i]['id'] ?>&action=fileClose">File Close</a></td>				     				
				     				</tr>
